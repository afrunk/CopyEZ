...
from app.models.wechat import WeChatUser, WeChatMessage, WeChatLoginHistory  # noqa: F401
...
from app.modules.wechat import wechat_auth_bp, wechat_chat_bp, wechat_upload_bp, avatar_bp, wechat_settings_bp, me_bp, wechat_push_bp
...
app.register_blueprint(wechat_push_bp)
...
def ensure_schema():
    ...
    # 登录历史是新增表，只在不存在时创建，不触碰现有聊天表。
    if "wechat_login_history" not in table_names:
        db.create_all()

    # wechat_push_subscriptions 表自建（Phase 7A：Web Push 提醒）
    if "wechat_push_subscriptions" not in table_names:
        from app.models.wechat import WeChatPushSubscription  # noqa: F401
        db.create_all()
